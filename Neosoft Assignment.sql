create database Neosoft_Aakash
...
Alter PROCEDURE stp_Emp_GetEmployeeByID
...
BEGIN
	SELECT COUNT(1)
	FROM EmployeeMaster 
    WHERE (@EmployeeCode is Null and  EmailAddress = @EmailAddress)
...
BEGIN
    SELECT COUNT(1)
    FROM EmployeeMaster
    WHERE (@EmployeeCode is Null and MobileNumber = @MobileNumber)
...
BEGIN
	SELECT COUNT(1)
	FROM EmployeeMaster
	WHERE (@EmployeeCode is Null and PanNumber = @PanNumber)
...
BEGIN
    SELECT COUNT(1)
    FROM EmployeeMaster
    WHERE (@EmployeeCode is Null and PassportNumber = @PassportNumber)
...
exec stp_Emp_CheckDuplicateEmail 'FateStayNight@rediffmail.com'
	...
	Declare @EmployeeCode varchar(10) = Null;
...
exec stp_Emp_CheckDuplicateEmail 'FateStayNight@rediffmail.com','007';
